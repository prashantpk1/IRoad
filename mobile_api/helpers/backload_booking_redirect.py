...
def should_pivot_shipment_to_backload_booking(
    *,
    ...
    if line in {'backload', 'inbound'}:
        return False
    if not policy.is_shipment_execution_complete(shipment):
        return False
    shipments = policy.sorted_countable_shipments(_load_booking_shipments(booking))
    if not policy.is_backload_leg_pending(booking, shipments):
        return False
    return policy.driver_owns_backload_leg(driver, booking)
