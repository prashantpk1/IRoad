...
class HardPODReceipt(models.Model):
    """Physical POD document receipt metadata (Hard POD)."""

    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    bundle = models.ForeignKey(
        PODCaptureBundle,
        on_delete=models.CASCADE,
        related_name='hard_pod_receipts',
    )
    tenant_schema = models.CharField(max_length=100, db_index=True)
    shipment_id = models.CharField(max_length=64, db_index=True)
    driver_id = models.CharField(max_length=64, db_index=True)
    document_serial = models.CharField(max_length=128, blank=True, default='')
    document_reference = models.CharField(max_length=128, blank=True, default='')
    receiver_name = models.CharField(max_length=200, blank=True, default='')
    receiver_identity_ref = models.CharField(max_length=128, blank=True, default='')
...
class HardPODCustodyEvent(models.Model):
    """Append-only Hard POD custody chain event."""
    ...
    bundle = models.ForeignKey(
        PODCaptureBundle,
        on_delete=models.CASCADE,
        related_name='hard_pod_custody_events',
    )
...
class HardPODVerification(models.Model):
    """Supervisor verification for Hard POD."""

    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    bundle = models.ForeignKey(
        PODCaptureBundle,
        on_delete=models.CASCADE,
        related_name='hard_pod_verifications',
    )
    tenant_schema = models.CharField(max_length=100, db_index=True)
    shipment_id = models.CharField(max_length=64, db_index=True)
    supervisor_id = models.CharField(max_length=64, blank=True, default='')
    supervisor_label = models.CharField(max_length=200, blank=True, default='')
